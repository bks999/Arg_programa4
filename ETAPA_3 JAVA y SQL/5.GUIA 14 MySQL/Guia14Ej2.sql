...
-- ascendente y en segundo lugar por el precio de forma descendente. (revisar)
...
-- Consultas Multitabla ( CON SOLO JOIN SE PUEDE)
-- 1. Devuelve una lista con el código del producto, nombre del producto, código del fabricante
-- y nombre del fabricante, de todos los productos de la base de datos.
SELECT producto.codigo, producto.nombre as producto, producto.codigo_fabricante, fabricante.nombre as fabricante FROM producto LEFT JOIN fabricante ON producto.codigo_fabricante = fabricante.codigo;
...
SELECT producto.nombre, producto.precio, fabricante.nombre FROM producto INNER JOIN fabricante ON producto.codigo_fabricante = fabricante.codigo WHERE producto.precio = (SELECT MIN(precio) FROM producto);
...
WHERE codigo IN (SELECT codigo_fabricante FROM producto WHERE codigo_fabricante !=(select codigo from fabricante where nombre = "lenovo")
GROUP BY codigo_fabricante
HAVING COUNT(codigo_fabricante)= (SELECT COUNT(codigo_fabricante) From producto WHERE codigo_fabricante =(select codigo from fabricante where nombre = "lenovo")));